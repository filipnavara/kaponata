...
namespace Kaponata.iOS.NotificationProxy
{
    ...
    public class NotificationProxyClientFactory : ServiceClientFactory<NotificationProxyClient>
    {
        ...
        {
            var protocol = await this.StartServiceAsync(serviceName, startSession: serviceName == NotificationProxyClient.ServiceName, cancellationToken);
            return new NotificationProxyClient(protocol);
        }
    }
}
